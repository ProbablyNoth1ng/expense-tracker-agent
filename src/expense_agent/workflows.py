...
def _fetch_node(services: Any, state: SyncState) -> SyncState:
    ...
    for account_id in state["account_ids"]:
        start = services.store.get_cursor(account_id)
        if start is None:
            local_start = datetime.combine(
                services.settings.initial_sync_date,
                time.min,
                tzinfo=ZoneInfo(services.settings.timezone),
            )
        ...
        for raw in services.monobank.iter_statements([account_id], start, now):
            ...
            decision = services.categorizer.categorize(normalized.merchant, normalized.mcc, amount_pln)
            proposals.append(
                ChangeProposal(
                    id=str(uuid.uuid4()),
                    action="ADD",
...
def _stage_node(services: Any, state: SyncState) -> SyncState:
    ...
    services.notifier.notify(
        "Expense agent sync",
        f"New: {state.get('new_count', 0)}; synced: {state.get('applied', {}).get('synced', 0)}",
    )
    return state
